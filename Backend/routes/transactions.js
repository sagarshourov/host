...
router.get('/:transactionId/complete', async (req, res) => {
    ...
    try {
        ...
        const initializeTasksQuery = `
            INSERT INTO task_value (task_id, offer_id, status)
        ...
        `;
        await client.query(initializeTasksQuery, [transactionId]);
        ...
        const phasesQuery = `
            ...
            ORDER BY sort_order
        `;
        const phases = await client.query(phasesQuery);
        ...
        `;
        const steps = await client.query(stepsQuery, [transactionId]);
        ...
        `;
         const tasks = await client.query(tasksQuery, [transactionId]);
...
router.patch('/:transactionId/tasks/:taskId/status', async (req, res) => {
    ...
    try {
        ...
        const taskResult = await client.query(taskExistsQuery, [taskId]);
        ...
        const insertStatusQuery = `
            INSERT INTO task_value (task_id, offer_id, status, changed_by, notes)
            VALUES ($1, $2, $3, $4, $5)
            RETURNING *
        `;
        const statusResult = await client.query(insertStatusQuery, [
            taskId,
            transactionId,
            status,
            userId || null,
        ...
        const updatedTaskQuery = `
            ...
            WHERE tv.id = $1
        `;
        const updatedTask = await client.query(updatedTaskQuery, [statusResult.rows[0].id]);
        ...
        if (status === 'completed') {
            ...
            `;
            const checkResult = await client.query(checkStepQuery, [transactionId, stepId]);
            ...
            if (incompleteCount === 0) {
                ...
                `;
                const stepResult = await client.query(updateStepQuery, [stepId]);
